...
- **Give-up and wrong-answer support** — after your first calculation, press `G` to give up and see the saved solution; a wrong final answer also shows the solution automatically.
...
| `E` | Return to the main menu — any time |

A vertical line on the left edge shows which half of the screen is active: **blue** (top) for the card row, **red** (bottom) for the working area — `Q`, `A`, `ENT`, and the `O`/`P` swap only respond while the working area is focused.

After the first `ENT`, **G** becomes available. Pressing it gives up the current attempt, immediately displays the saved solution, and then offers `R D E`.
