{ ============================================================
  ShowCard_68 - lower focus extended from user baseline
  ChatGPT - 03-Sep-2026 23:43 Israel time
...
{ KRYPTO Release 1.0 - Give Up version
  Modified by ChatGPT / OpenAI - 2026-09-07 15:40 +03:00
...
{$l divmod.asm}
{$m 2048}
{$l beepfx2_cards_test.asm} { machine code to be used to play sound effects.}
...
VAR
  ...
  GiveUpAvailable: Byte;           { G becomes available after the first = }
...
VAR K,N,Code,OuterCode,PS,PI,CN: Byte;
    C,X,R,PV: Integer;
BEGIN
...
{ ------------------------------------------------------------
  RedrawLowerCursorTail
  Restores the two bottom characters of the red lower-area focus
  cursor.  These characters are drawn in Channel 1 and are erased
  whenever ClearCommandLines clears the two command rows.

  Called after such clears when the lower area still has focus.
  SetAttr is used rather than TextColor so the cursor always has
  the correct red/bright attributes with FLASH off, regardless of
  the attributes used by the preceding message.
  ------------------------------------------------------------ }

PROCEDURE RedrawLowerCursorTail;
BEGIN
  IF Focus = 1 THEN
  BEGIN
    SetAttr(4,2,1,0);
...
BEGIN
  ...
  BEGIN
    {SoundEffect(4);}
    BlitCard(CardXPos[i], CardYAt(i),
             CardBackPattern, CardBackRows, CardBackAttr)
  END
END;
...
BEGIN
  ...
  BEGIN
    GotoXY(7,2);
    Write('ENT O/P Q/A C R D E')
  END;

  { G is deliberately added in one place, rather than duplicating
    all five menu strings. }
  IF GiveUpAvailable = 1 THEN
  BEGIN
    GotoXY(28,2);
    Write('G')
  END;
...
{ Show the saved solution and finish this attempt.
  Used both by G (give up) and automatically after WRONG. }
PROCEDURE SolutionEndMenu;
VAR K:Char;
BEGIN
  ShowSavedSolution;
  ClearCommandLines;
...
VAR
  temp: Integer;
  newIndex, i, value, search, found, isFinal: Byte;
  k: Char;
BEGIN
  cursorIndex := 0;
  ChoosingOp := 0;
  ...
  REPEAT
    ...
    { G is available only after at least one calculation. }
    IF k = 'G' THEN
      IF GiveUpAvailable = 1 THEN
        SolutionEndMenu;
...
{ ------------------------------------------------------------
  Main program with main menu.

  1 = play game
  2 = instructions (placeholder screen for now)
  3 = about        (placeholder screen for now)
  Q = quit to BASIC

  E inside the game now returns to this main menu.
  ------------------------------------------------------------ }
BEGIN
  Randomize;
  REPEAT
    ...
    Write('3 ABOUT');
    GotoXY(9,15);
    Write('Q QUIT TO BASIC');
    GotoXY(8,19);
    Write('Press 1, 2, 3 or Q');

    REPEAT
      EndKey := UpCase(ReadKey)
    UNTIL (EndKey = '1') OR (EndKey = '2') OR
          (EndKey = '3') OR (EndKey = 'Q');
    ...
    ELSE
      ...
      BEGIN
        ...
        GotoXY(2,4);
        Write('Use all 5 cards exactly once');
        GotoXY(2,5);
        Write('to make the target number.');
        GotoXY(2,7);
        Write('+  -  *  /   No fractions');
        GotoXY(2,8);
        Write('Negative results are allowed.');

        GotoXY(2,10);
        Write('O/P  Move');
        GotoXY(2,11);
        Write('S    Select / move card');
        GotoXY(2,12);
        Write('C    Change area');
        GotoXY(2,13);
        Write('Q/A  Change operator');
        GotoXY(2,14);
        Write('ENT  Calculate');
        GotoXY(2,15);
        Write('D    Redo same deal');
        GotoXY(2,16);
        Write('R    Reshuffle');
        GotoXY(2,17);
        Write('G    Give up / solution');
        GotoXY(2,18);
        Write('E    Main menu');
      ...
      ELSE
        ...
        BEGIN
          ...
          GotoXY(4,6);
          Write('KRYPTO for ZX Spectrum 48K');
          GotoXY(11,7);
          Write('Release 1.0');
          GotoXY(2,9);
          Write('Written in Pascal with PASTA80');
          GotoXY(10,10);
          Write('By Micky Adler');
          GotoXY(14,11);
          Write(CHR(127));
          Write(' 2026');

          GotoXY(5,14);
          Write('Development assistance:');
          GotoXY(8,16);
          Write('ChatGPT / OpenAI');
          GotoXY(7,17);
          Write('Claude / Anthropic');
